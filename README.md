...
## APIs In Use
  ...
  * [OpenLibrary](https://openlibrary.org/developers/api) - This project originally utilized GoodReads' API, but on top of them depracating said (not very good, frankly) API, GoodReads as a platform is totally stagnant and obviously not interested in being a good steward of data. OpenLibrary, meanwhile, totally is invested in just that, and also it returns its data in JSON because it's the 21st century and not 1642, *Amazon*.
  ...
  * [Discogs](https://www.discogs.com/developers/) is a massive repository of information for released music, and my source of truth for tracking CDs as well as vinyls. Discogs' information is generally limited towards things that have gotten a physical release (excluding things such as Soundcloud or digital-only Bandcamp releases), but since this project is built to handle my physical collection, this constraint is a non-issue for me.
  * [Jikan](https://jikan.moe/) is a FOSS Restful API built on top of the massive collection of anime data hosted at MyAnimeList. This will allow me to cultivate a distinct category from movies for my small but non-zero collection of physical anime releases.
